...
export class TestExecutor {
   ...
   */
  public runScenario(options: TestExecutionOptions): void {
   ...
   */
  public runFeatureFile(options: FeatureExecutionOptions): void {
    const workingDir = this.getWorkingDirectory();
    const behaveCommand = this.config.behaveCommand;
   ...
   */
  public runFeatureFileLegacy(filePath: string): void {
    this.runFeatureFile({ filePath });
  }
...
`;
   ...
   */
  public executeTestWithOutput(options: TestExecutionOptions): TestRunResult {
    const startTime = Date.now();

    try {
      this.runScenario(options);
